...
class StockStrategyEngine(QCAlgorithm):
    """
    FYP US Equity Trading Engine — 5 Mean-Reversion Strategy Families
    ==================================================================
    ...
    Supports three trading styles:
      - day_trade:  5-min bars, session filter 9:30-16:00 ET, force flat 15:55 ET
      - swing:      60-min bars, no session filter, no force flat
      - long_term:  daily bars, no session filter, no force flat
    ...
    def initialize(self):
        ...
        self.max_trades_per_day = max(param_int("max_trades_per_day", 8), 1)

        # --- Trading Style ---
        self.trading_style = param_str("trading_style", "day_trade").lower()
        self._apply_trading_style_defaults(param_int, param_float)

        # --- Asset Setup ---
        pairs_str = param_str("pairs", "SPY")
        ...
        self.max_hold_bars = max(param_int("max_hold_bars", self._default_max_hold_bars), 1)
        ...
        # --- Add Equities ---
        for ticker in pairs:
            security = self.add_equity(ticker, Resolution.MINUTE, Market.USA)
            ...
            # Consolidator — stocks use TradeBar
        ...
        self.debug(
            f"[INIT] Family={family_name} | Mode={self.mode} | Style={self.trading_style} | "
            f"Tickers={pairs} | Bar={self.bar_minutes}m | "
            f"Cash=${self.starting_cash:,.0f} | Risk={self.risk_per_trade_pct}% | "
            f"SessionFilter={self.use_session_filter} | ForceFlat={self.use_force_flat}"
        )

    def _apply_trading_style_defaults(self, param_int, param_float):
        """Set bar_minutes, session/flat filters based on trading_style."""

        if self.trading_style == "swing":
            default_bar = 60
            default_session = 0
            default_force_flat = 0
            self._default_max_hold_bars = 50
            default_session_start = 0
            default_session_end = 23
            default_last_entry_hour = 23
            default_last_entry_minute = 59
            default_flat_hour = 23
            default_flat_minute = 59

        elif self.trading_style == "long_term":
            default_bar = 1440
            default_session = 0
            default_force_flat = 0
            self._default_max_hold_bars = 100
            default_session_start = 0
            default_session_end = 23
            default_last_entry_hour = 23
            default_last_entry_minute = 59
        ...
        else:  # day_trade (default)
            default_bar = 5
            default_session = 1
            default_force_flat = 1
            self._default_max_hold_bars = 20
            default_session_start = 9
            default_session_end = 16
            default_last_entry_hour = 15
            default_last_entry_minute = 30
            default_flat_hour = 15
            default_flat_minute = 55

        self.bar_minutes = max(param_int("bar_minutes", default_bar), 1)

        self.use_session_filter = param_int("use_session_filter", default_session) == 1
        self.session_start_hour = param_int("session_start_hour", default_session_start)
        self.session_start_minute = param_int("session_start_minute", 30 if self.trading_style == "day_trade" else 0)
        self.session_end_hour = param_int("session_end_hour", default_session_end)
        self.last_entry_hour = param_int("last_entry_hour", default_last_entry_hour)
        self.last_entry_minute = param_int("last_entry_minute", default_last_entry_minute)
        self.use_force_flat = param_int("use_force_flat", default_force_flat) == 1
        self.force_flat_hour = param_int("force_flat_hour", default_flat_hour)
        self.force_flat_minute = param_int("force_flat_minute", default_flat_minute)
    ...
    def _in_session(self):
        ...
        session_start_min = self.session_start_hour * 60 + self.session_start_minute
        session_end_min = self.session_end_hour * 60
    ...
    def _loss_per_unit_usd(self, sym, stop_dist):
        """For USD-denominated equities, loss per share equals the stop distance."""
        if stop_dist <= 0:
            return 0.0
        return stop_dist
    ...
    def on_end_of_algorithm(self):
        ...
        self.set_runtime_statistic("Style", self.trading_style)
        ...
        self.debug(
            f"[FINAL] Family={family_name} | Style={self.trading_style} | "
